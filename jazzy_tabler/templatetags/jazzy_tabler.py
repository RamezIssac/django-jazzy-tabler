...
@register.simple_tag
def get_jazzy_settings(request: WSGIRequest) -> Dict[str, Any]:
...
@register.simple_tag
def get_jazzy_ui_tweaks() -> Dict[str, Any]:
    return get_ui_tweaks()


@register.simple_tag
def get_jazzy_version() -> str:
    return str(version)
...
@register.simple_tag
def jazzy_paginator_number(change_list: ChangeList, i: Union[int, str]) -> SafeText:
    html_str = ""
    start = i == 1
    ...
    if start:
        ...
        <li class="page-item previous {disabled}">
            <a class="page-link" href="{link}" tabindex="0">&laquo;</a>
        </li>
    ...
    if current_page:
        ...
        <li class="page-item active">
            <a class="page-link" href="javascript:void(0);" tabindex="0">{num}</a>
        </li>
        """.format(num=i)
    elif spacer:
        html_str += """
        <li class="page-item disabled">
            <a class="page-link" href="javascript:void(0);" tabindex="0">&hellip;</a>
        </li>
        """
    else:
        ...
        html_str += """
        <li class="page-item">
            <a href="{query_string}" class="page-link" tabindex="0">{num}</a>
        </li>
        """.format(num=i, query_string=query_string)

    if end:
        link = change_list.get_query_string({PAGE_VAR: change_list.page_num + 1}) if change_list.page_num < i else "#"
        html_str += """
        <li class="page-item next {disabled}">
            <a class="page-link" href="{link}" tabindex="0">&raquo;</a>
        </li>
        """.format(link=link, disabled="disabled" if link == "#" else "")
...
@register.simple_tag
def jazzy_list_filter(cl: ChangeList, spec: ListFilter) -> SafeText:
...
@register.filter
def has_jazzy_setting(settings: Dict[str, Any], key: str) -> bool:
...
def get_sections(
    ...
    if hasattr(admin_form.model_admin, "jazzy_section_order"):
        fieldsets = order_with_respect_to(
            fieldsets, admin_form.model_admin.jazzy_section_order, getter=lambda x: x.name
        )
...
@register.simple_tag
def action_message_to_list(action: LogEntry) -> List[Dict[str, Any]]:
